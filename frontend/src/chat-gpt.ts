...
export class ChatGPT extends LitElement {
  ...
  async performPostRequest(endpoint: string, body: Thread | FormData): Promise<any> {
    this.loading = true;
    let headers = {};
  ...
  async submit() {
    switch (this.thread.selected_engine) {
  ...
  async runChatReq() {
    ...
    const reqBody: Thread = this.thread;
    try {
      const response = await this.performPostRequest('/', reqBody);

      const reader = response.body.getReader();
      let message = '';
      const nextIndex = this.thread.history.length;
  ...
  async runAutoReq() {
    ...
    const reqBody: Thread = this.thread;
    try {
      const response = await this.performPostRequest('/auto', reqBody);
      ...
      if (response.headers.get('content-type')?.includes('application/json')) {
        ...
        // Assuming `json.content` is the part of the response you want to display
        this.addToHistory('assistant', json);
      ...
      console.error(err);
      this.addToHistory('assistant', `auto api error: ${err.message}. Try again.`);
    } finally {
      this.loading = false;
    }
  }
  ...
  async runImageReq() {
    // const engine = this.thread.selected_engine;
    // const element = this.textareaEl;
    // const prompt = element.value;
    // element.value = '';
    // await this.addToHistory('user', prompt);
  ...
  deleteItem(index: number) {
    const reversedIndex = this.thread.history.length - index - 1;
  ...
  render() {
    ...
    return html`
      <div class="history-outer">
        ...
        ${[...this.thread.history].reverse().map(
          (item, i) => html`
      ...
      <div class="inputs-outer">
        <div class="inputs-inner">
          <textarea
            ?disabled=${this.thread.selected_engine === 'gpt-4-vision-preview'}
          ...
          <div class="buttons">
            ${this.miniPreviewImageURL
              ...
              : nothing}
            ${this.thread.selected_engine?.includes('vision')
              ? html`
                  <input
                    type="file"
                    accept="image/*"
